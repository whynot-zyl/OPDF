    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        if test:
            ...
            if task == 'mprc' or task == 'wnli':
                ...
                # 生成 InputExample 对象
                examples = []
                # for i in range(len(example['question1'])):
                #     if i == 0:
                #         continue
                #     guid = "%s-%s" % ('test', i)
                #     text_a = example['question1'][i]
                #     text_b = example['question2'][i]
                #     label = label_list[example['label'][i]]
                #     examples.append(InputExample(guid=guid, text_a=text_a, text_b=text_b, label=label))
                examples = [
                    InputExample(guid=f"test-{i}", 
                                text_a=example['question1'][i], 
                                text_b=example['question2'][i], 
                                label=label_list[example['label'][i]])
                    for i in range(1, len(example['question1']))
                ]
                ...
                examples = [
                    InputExample(guid=f"test-{i}", 
                                   text_a=example['question'][i], 
                                text_b=example['sentence'][i], 
                                label=label_list[example['label'][i]])
                    for i in range(1, len(example['question']))
                ]
