...
def train(args, train_dataset, model, tokenizer):
    ...
    # 对scc_layer进行MPO
    print("########################################################################################`#################")
    print("bert参数量："+str(sum(p.numel() for p in model.bert.encoder.layer.parameters() if p.requires_grad)))
    print("#########################################################################################################")
        # 获取name
    basic_choose_name = {}
    gradient_mask = dict()
    ...
    for name, params in model.bert.encoder.layer.named_parameters():
        if name in choose_name:
            print(name)
            model.bert.encoder.fine_grained_decomposition2(name)
    print("#########################################################################################################")
    ...
    for name, params in model.bert.encoder.scc_layer.named_parameters():
        if name in choose_name:
            print(name)
            model.bert.encoder.fine_grained_decomposition(name)
    print("#########################################################################################################")
    print("参数量："+str(sum(p.numel() for p in model.bert.encoder.scc_layer.parameters() if p.requires_grad)))
    # 存储为txt文件
    with open("/home/zhanyuliang/Project/DistillingMPO/OPF/BERT-of-Theseus/bert_of_theseus/scc_layer.txt","w") as f:
    ...
    print("#########################################################################################################")
    print("优化器参数量："+str(sum(p.numel() for group in optimizer.param_groups for p in group['params'])))
    print("#########################################################################################################")
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            outputs,loss_layer = model(**inputs)
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        logger.info("  model size = %d", sum(p.numel() for p in model.bert.encoder.scc_layer.parameters() if p.requires_grad))
        eval_loss = 0.0
        nb_eval_steps = 0
        preds = None
        out_label_ids = None
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False, test=False):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        if test:
            ...
            elif task == 'sts-b' or task == 'rte' or task == 'sst-2' or task == 'qqp' or task == 'cola' or task == 'qnli' or task in ['mnli', 'mnli-mm']:
...
def main():
    ...
    if args.do_train:
        ...
        global_step, tr_loss, best_model = train(args, train_dataset, model, tokenizer)
        logger.info(" global_step = %s, average loss = %s", global_step, tr_loss)
    model.load_state_dict(best_model)
    print("#########################################################################################################")
    print("train结束参数量："+str(sum(p.numel() for p in model.bert.encoder.scc_layer.parameters() if p.requires_grad)))
    print("#########################################################################################################")
    ...
    #     model.to(args.device)
    print("#########################################################################################################")
    print("存储结束参数量："+str(sum(p.numel() for p in model.bert.encoder.scc_layer.parameters() if p.requires_grad)))
    print("#########################################################################################################")
    # Evaluation
    results = {}
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            print("#########################################################################################################")
            print("evaluate开始参数量："+str(sum(p.numel() for p in model.bert.encoder.scc_layer.parameters() if p.requires_grad)))
            print("#########################################################################################################")
            model.to(args.device)
