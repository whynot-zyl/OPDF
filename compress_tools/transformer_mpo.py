...
def get_MPOtensor_shape():
    '''
    input: mpo_input_shape, mpo_output_shape
    output: T1_shape, T2_shape, ..., Tn_shape
    '''
    pass
...
class DeepSpeedTransformerLayer(nn.Module):
    ...
    def __init__(self, config, initial_weights=None, initial_biases=None):
        ...
        if initial_weights is None and initial_biases is None:
            ...
            if self.config.mpo_layers.lower() != "nompo":
                self.attn_qkvw = nn.ParameterList([nn.Parameter(torch.Tensor(*i)) for i in get_MPOtensor_shape(self.config.attention_input_shape,
                                                                                                                    self.config.attention_output_shape)])
                self.inter_w = nn.ParameterList([nn.Parameter(torch.Tensor(*i)) for i in get_MPOtensor_shape(self.config.FFN_1_input_shape,
                                                                                                                    self.config.FFN_1_output_shape)])
                self.output_w = nn.ParameterList([nn.Parameter(torch.Tensor(*i)) for i in get_MPOtensor_shape(self.config.FFN_2_input_shape,
                                                                                                                self.config.FFN_2_output_shape)])
        else:
            # For testing only.
